...
<nav class="navbar navbar-default navbar-fixed-top" role="navigation">
  ...
  <a href="assignTask.jsp" class="list-group-item active">Assign Tasks</a>
</div> 
                    </div>
                </div>
                <div class="col-md-9 ">
...
<a href="assign.jsp"><button type="button" class="btn btn-danger pull-right">Assign Task</button></a>
 ...
 <table id="myTable">
  <tr>
   <!--When a header is clicked, run the sortTable function, with a parameter, 0 for sorting by names, 1 for sorting by country:-->  
    ...
    <th onclick="sortTable(3)">Scheduled Date</th>
  </tr>
  
  <% 
  ArrayList<taskViewList> taskList = new ArrayList<taskViewList>(); 
  
  rs = stmt.executeQuery("select td.task, u.name, t.assigned_by, t.scheduled_date from tasks t inner join task_details td on td.task_id = t.task inner join users u on u.user_id = t.assgined_to");
	while(rs.next())
	{
		taskViewList data = new taskViewList();
		data.setTask(rs.getString(1));
		data.setAssignedTo(rs.getString(2));
		data.setAssignedBy(rs.getString(3));
		data.setScheduledDate(rs.getString(4));
		taskList.add(data);
	}
  ...
  <% for (int i=0; i < taskList.size(); i++)
  {
	  %>
	  <tr>
	  <td><%out.print(taskList.get(i).getTask()); %></td>
	  <td><%out.print(taskList.get(i).getAssignedTo() ); %></td>
	  <td><%out.print(taskList.get(i).getAssignedBy() ); %></td>
	  <td><%out.print(taskList.get(i).getScheduledDate() ); %></td>
	  </tr>
	  <%
  }
  %>
